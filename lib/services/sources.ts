...
export const sources = {
  async listForUser(
    ...
    console.log(`[sources] listForUser temporarily disabled - using backend API instead: ${userId}`)
  ...
  async add(userId: string, name: string, priority: number = 0): Promise<SourceItem> {
    ...
    // Temporary mock response until backend API is implemented
    console.log(`[sources] add temporarily disabled - using backend API instead: ${userId}, ${name}`)
  ...
  async remove(userId: string, id: string): Promise<void> {
    ...
    console.log(`[sources] remove temporarily disabled - using backend API instead: ${userId}, ${id}`)
  },
  ...
  async addBatch(userId: string, names: string[]): Promise<SourceItem[]> {
    ...
    // Temporary mock response until backend API is implemented
    console.log(`[sources] addBatch temporarily disabled - using backend API instead: ${userId}`)
    
    return names.map(name => ({
      id: `mock_${name}`,
      name: name.replace(/^r\//, '').trim().toLowerCase(),
      title: `r/${name.replace(/^r\//, '').trim().toLowerCase()}`,
      nsfw: false,
      isEnabled: true,
      priority: 0,
      lastUsedAt: null,
  ...
  async reorder(
    ...
    // Temporary mock response until backend API is implemented
    console.log(`[sources] reorder temporarily disabled - using backend API instead: ${userId}`)
    
    return items.map(item => ({
      id: item.id,
      subreddit: {
        name: `mock_${item.id}`,
        title: `Mock Subreddit ${item.id}`,
      },
      priority: item.priority,
    }))
  },
